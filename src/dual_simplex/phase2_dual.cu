...
void build_basis_on_device(i_t m, const i_t *d_A_col_start, const i_t *d_A_row_ind,
    ...
    thrust::device_ptr<i_t> dev_ptr(d_B_row_ptr);
...
i_t factorize_and_repair(i_t m, i_t n, const csc_matrix_t<i_t, f_t> &A,
    ...
    CUDA_CALL_AND_CHECK(cudaMalloc(&d_B_col_ind, B_csr.nz_max * sizeof(i_t)),
    ...
    CUDA_CALL_AND_CHECK(
        cudaMemcpy(d_B_col_ind, B_csr.j.data(), B_csr.nz_max * sizeof(i_t), cudaMemcpyHostToDevice),
        "cudaMemcpy d_B_col_ind");
    CUDA_CALL_AND_CHECK(
        cudaMemcpy(d_B_values, B_csr.x.data(), B_csr.nz_max * sizeof(f_t), cudaMemcpyHostToDevice),
        "cudaMemcpy d_B_values");
...
//
template <typename i_t, typename f_t>
void move_basis_to_device(cudssMatrix_t *d_B_matrix, const csc_matrix_t<i_t, f_t> &A,
                          const std::vector<i_t> &basic_list, cudssHandle_t &dss_handle) {
    // Assemble B in CSR format
    const i_t m = A.m;
    csr_matrix_t<i_t, f_t> B_csr;
 ...
 */
template <typename i_t, typename f_t>
void move_A_to_device(cudssMatrix_t *d_A_matrix, const csc_matrix_t<i_t, f_t> &A,
                      cudssHandle_t &dss_handle) {
    // transform A to CSR
    csr_matrix_t<i_t, f_t> A_csr;
    ...
    i_t *d_A_col_ind;
    f_t *d_A_values;
    CUDA_CALL_AND_CHECK(cudaMalloc(&d_A_row_ptr, (A_csr.m + 1) * sizeof(i_t)),
                        "cudaMalloc d_A_row_ptr");
    CUDA_CALL_AND_CHECK(cudaMalloc(&d_A_col_ind, A_csr.nz_max * sizeof(i_t)),
                        "cudaMalloc d_A_col_ind");
    CUDA_CALL_AND_CHECK(cudaMalloc(&d_A_values, A_csr.nz_max * sizeof(f_t)),
                        "cudaMalloc d_A_values");

    // Copy data to device
    // Row pointers
    CUDA_CALL_AND_CHECK(cudaMemcpy(d_A_row_ptr, A_csr.row_start.data(), (A_csr.m + 1) * sizeof(i_t),
                                   cudaMemcpyHostToDevice),
                        "cudaMemcpy d_A_row_ptr");
    // Column indices
    CUDA_CALL_AND_CHECK(
        cudaMemcpy(d_A_col_ind, A_csr.j.data(), A_csr.nz_max * sizeof(i_t), cudaMemcpyHostToDevice),
        "cudaMemcpy d_A_col_ind");
    // Non-zero values
    CUDA_CALL_AND_CHECK(
        cudaMemcpy(d_A_values, A_csr.x.data(), A_csr.nz_max * sizeof(f_t), cudaMemcpyHostToDevice),
        "cudaMemcpy d_A_values");

    // Create cudssMatrix_t for A
    // Note: CUDSS expects column-major format, but we provide CSR format.
    cudssStatus_t dss_status = CUDSS_STATUS_SUCCESS;
    CUDSS_CALL_AND_CHECK(cudssMatrixCreateCsr(d_A_matrix, A_csr.m, A_csr.n, A_csr.nz_max,
                                              d_A_row_ptr, NULL, d_A_col_ind, d_A_values,
                                              CUDA_R_32I, CUDA_R_64F, CUDSS_MTYPE_GENERAL,
                                              CUDSS_MVIEW_FULL, CUDSS_BASE_ZERO),
                         dss_status, "cudssMatrixCreateCsr for A");
}
...
dual::status_t dual_phase2_cu(i_t phase, i_t slack_basis, f_t start_time,
    ...
    // cudssMatrix_t d_A_matrix;
    // move_A_to_device(&d_A_matrix, lp.A, dss_handle);
    //
    // cudssMatrix_t d_B_matrix;
