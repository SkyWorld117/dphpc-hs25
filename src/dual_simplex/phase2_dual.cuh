...
void pinv_solve(
    ...
    CUDA_CALL_AND_CHECK(cudaMemcpy(
        d_rhs,
        rhs.data(),
        m * sizeof(f_t),
...
template <typename i_t, typename f_t>
void compute_primal_variables(cublasHandle_t& cublas_handle,
                              f_t* d_B_pinv,
...
template <typename i_t, typename f_t>
void compute_delta_y(cublasHandle_t& cublas_handle,
                     f_t* d_B_pinv,
                     i_t basic_leaving_index,
                     i_t direction,
...
template <typename i_t, typename f_t>
void adjust_for_flips(cublasHandle_t& cublas_handle,
                      f_t* d_B_pinv,
                      const std::vector<i_t>& basic_list,
                      const std::vector<i_t>& delta_z_indices,
                      std::vector<i_t>& atilde_index,
...
//   ft.b_solve(rhs_sparse, scaled_delta_xB_sparse, utilde_sparse);
    phase2::pinv_solve(cublas_handle, d_B_pinv, rhs_sparse, scaled_delta_xB_sparse, (i_t)(basic_list.size()), false);
                        ...
                        lp_solution_t<i_t, f_t>& sol)
{
  ...
  if (perturbation > 1e-6 && phase == 2) {
    ...
    phase2::compute_dual_solution_from_basis(
      lp, handle, d_B_pinv, basic_list, nonbasic_list, unperturbed_y, unperturbed_z);
    {
