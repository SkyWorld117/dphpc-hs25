...
from dualsimplex import dualsimplex, solve_lp
...
def test_mps_file(mps_path: str, device='cpu', max_iter=10000, verbose=True) -> MPSTestResult:
    ...
    try:
        ...
        # Interpret ierr and populate result fields
        if ierr == 0 and X is not None:
            result.status = 'optimal'
            # Compute objective value: maximize c^T x
            result.objective_value = float(torch.dot(c, X).item())
            result.iterations = 0
        else:
            result.status = f'error_{ierr}' if ierr is not None else 'error'
        ...
        if verbose:
            ...
            if X is not None:
                ...
                # Non-negativity check for primal variables (if expected)
                non_negativity = bool(torch.all(x >= -1e-6).item())
